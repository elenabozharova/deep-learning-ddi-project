"""
Joint-pair-encoding extension — ChemBERTa MLP on jointly-encoded drug-pair
embeddings: test-set evaluator.

Must be run AFTER train_chemberta_mlp_joint_pair.py has produced:
    outputs/polyllm/chemberta_joint_pair_training/checkpoints/best_model.pt
    outputs/polyllm/chemberta_joint_pair_training/validation_threshold.json

Usage:
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_chemberta_mlp_joint_pair.py
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_chemberta_mlp_joint_pair.py --overwrite

The test split is evaluated exactly once. Metrics are identical in
definition to Milestone 6B/7/9c/9d for direct comparability; only the
reported "loss" value uses this run's own faithful BinaryFocalLossWithLogits
(reconstructed from training_config.json) rather than BCE, since that is
the loss this checkpoint was actually trained under. AP@50 reported here
is the original per-pair-axis metric; the paper-comparable per-label-axis
value is produced separately by recompute_ap_at_k.py.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate the faithful-training ChemBERTa MLP ablation on the test set.")
    p.add_argument("--overwrite", action="store_true", help="Overwrite existing test-set outputs.")
    return p.parse_args()
...
def check_prerequisites(overwrite: bool) -> None:
    ...
    if TEST_METRICS.exists() and not overwrite:
        raise FileExistsError(f"Test metrics already exist: {TEST_METRICS}\nPass --overwrite to re-evaluate.")
...
def validate_checkpoint_config(ckpt: dict, features: np.ndarray, labels: np.ndarray) -> None:
    ...
    if ckpt_in != features.shape[1]:
        raise ValueError(f"Checkpoint input_dim={ckpt_in} does not match feature dim={features.shape[1]}")
    if ckpt_out != labels.shape[1]:
        raise ValueError(f"Checkpoint output_dim={ckpt_out} does not match label dim={labels.shape[1]}")
...
def main() -> None:
    ...
    model = MultilabelMLP(
        ...
        negative_slope=train_config.get("leaky_relu_negative_slope", 0.1),
    ).to(device)

    ckpt = load_checkpoint(CHECKPOINT_PATH, model, device=device)
    validate_checkpoint_config(ckpt, features, labels)
    print(f"  checkpoint epoch={ckpt['epoch']}  val_macro_auprc={ckpt['val_macro_auprc']:.4f}")
    ...
    criterion = BinaryFocalLossWithLogits(
        ...
        alpha=train_config.get("focal_alpha", -1.0),
        label_smoothing=train_config.get("focal_label_smoothing", 0.2),
    )

    print("\nRunning test-set inference ...")
    _, test_probs, test_labels, test_pids = run_val_epoch(model, test_loader, criterion, device)
    ...
    assert np.array_equal(test_pids, np.sort(test_pids)), "test predictions are not sorted by pair_id"
    ...
    test_metrics = {
        ...
        "paper_ap50_definition_compatibility": "unresolved (see recompute_ap_at_k.py for the paper-axis value)",
