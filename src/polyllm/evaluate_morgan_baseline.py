"""
Milestone 6B — Morgan fingerprint MLP: test-set evaluator.

Must be run AFTER train_morgan_baseline.py has produced:
    outputs/baseline/morgan/checkpoints/best_model.pt
    outputs/baseline/morgan/validation_threshold.json

Usage:
    .venv-polyllm\Scripts\python.exe src/polyllm/evaluate_morgan_baseline.py
    .venv-polyllm\Scripts\python.exe src/polyllm/evaluate_morgan_baseline.py --overwrite

The test split is evaluated exactly once.  This script refuses to run if the
checkpoint or threshold file is missing, or if test outputs already exist
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate Morgan MLP on the test set.")
...
def check_prerequisites(overwrite: bool) -> None:
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run train_morgan_baseline.py first."
        )
...
def main() -> None:
    ...
    # Load features and labels (mmap — no copy into RAM)
    ...
    # Load training config and threshold
    ...
    # Build test DataLoader — no shuffle
    ...
    # Run inference — single evaluation of the test set
    ...
    # Verify pair-id ordering (should already be sorted by run_val_epoch)
    ...
    # -----------------------------------------------------------------------
    # Save test metrics JSON (last, to signal completion)
    # -----------------------------------------------------------------------
    ...
    assert set(["label_index", "side_effect_id", "auroc", "auprc",
