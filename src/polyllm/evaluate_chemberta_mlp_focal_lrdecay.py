"""
Milestone 9d — ChemBERTa MLP ablation (Focal loss + decayed LR): test-set evaluator.

Must be run AFTER train_chemberta_mlp_focal_lrdecay.py has produced:
    outputs/polyllm/chemberta_focal_lrdecay/checkpoints/best_model.pt
    outputs/polyllm/chemberta_focal_lrdecay/validation_threshold.json

Usage:
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_chemberta_mlp_focal_lrdecay.py
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_chemberta_mlp_focal_lrdecay.py --overwrite

The test split is evaluated exactly once. Metrics are identical in
definition to Milestone 6B/7/9c for direct comparability; only the
reported "loss" value uses this run's own BinaryFocalLossWithLogits
(reconstructed from training_config.json) rather than BCE, since that is
the loss this checkpoint was actually trained under.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate the Focal/LR-decay ChemBERTa MLP ablation on the test set.")
...
def check_prerequisites(overwrite: bool) -> None:
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(
            f"Best checkpoint not found: {CHECKPOINT_PATH}\nRun train_chemberta_mlp_focal_lrdecay.py first."
        )
