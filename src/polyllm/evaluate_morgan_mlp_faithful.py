"""
Controlled representation ablation — Morgan fingerprint MLP under the
Milestone 9e recipe: test-set evaluator.

Must be run AFTER train_morgan_mlp_faithful.py has produced:
    outputs/baseline/morgan_faithful_training/checkpoints/best_model.pt
    outputs/baseline/morgan_faithful_training/validation_threshold.json

Usage:
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_morgan_mlp_faithful.py
    .venv-polyllm/Scripts/python.exe src/polyllm/evaluate_morgan_mlp_faithful.py --overwrite

Metric definitions are identical to every other MLP evaluator in this repo
for direct comparability. AP@50 reported here is the original per-pair-axis
metric; the paper-comparable per-label-axis value is produced separately.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate the faithful-training Morgan MLP ablation on the test set.")
...
def main() -> None:
    ...
    test_metrics = {
        ...
        "paper_ap50_definition_compatibility": "unresolved (see recompute_faithful_ap_at_k.py for the paper-axis value)",
