"""
Milestone 9e — ChemBERTa MLP "faithful-training" ablation.

Goal (user-specified, 2026-08-09): determine whether matching the PolyLLM
authors' actual focal-loss formulation, batch size, and per-step learning-
rate schedule -- as opposed to Milestone 9d's approximate versions --
explains the remaining gap between our stable MLP results and the paper,
and/or explains M9d's training instability.

This is explicitly NOT an attempt to reproduce the authors' sequential
10-fold-with-weight-carryover training loop (see src/mlp/mlp.py at
github.com/sadrahkm/PolyLLM). It keeps our existing fixed 80/10/10
pair-random split, our existing early-stopping methodology (patience=10,
monitor=val_macro_auprc, restore best checkpoint), and evaluates on the
same held-out test split as every other MLP experiment in this repo. Only
three things change relative to Milestone 9d:

    1. Loss:  BinaryFocalLossWithLogits(gamma=2.0, alpha=-1 [disabled],
              label_smoothing=0.2) -- matches Keras
              BinaryFocalCrossentropy(label_smoothing=0.2) with
              apply_class_balancing left at its Keras default (False).
    2. Batch size: 32 (Keras model.fit() default -- the authors' code has
              `# batch_size=128` commented OUT, so it silently uses 32).
    3. LR schedule: keras_exponential_decay_lr(step, initial_learning_rate,
              decay_steps=1000, decay_rate=0.96, staircase=True), applied
              PER OPTIMIZER STEP (mini-batch), not per epoch as M9d did.

Output goes to outputs/polyllm/chemberta_faithful_training/ -- a new,
separate directory. Neither Milestone 7/9c's outputs/polyllm/chemberta/
nor Milestone 9d's outputs/polyllm/chemberta_focal_lrdecay/ are touched.

Official run:
    .venv-polyllm/Scripts/python.exe src/polyllm/train_chemberta_mlp_faithful.py
...
OUTPUT_DIR      = Path("outputs/polyllm/chemberta_faithful_training")
...
DEFAULT_CONFIG: dict = {
    ...
    "experiment_note": (
        "Milestone 9e faithful-training ablation. Same data, same ChemBERTa "
        "embeddings, same MultilabelMLP architecture (incl. the Milestone 9c "
        "slope=0.1 fix), same fixed 80/10/10 split as Milestone 7/9c/9d. "
        "Isolates loss formula + batch size + LR schedule mechanics against "
        "the authors' verified source (github.com/sadrahkm/PolyLLM), without "
        "adopting their sequential 10-fold protocol. See "
        "notes/deviations_from_paper.md Sec 1.9."
    ),
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Milestone 9e ablation: ChemBERTa MLP with faithful focal loss, batch size, and LR schedule."
    )
