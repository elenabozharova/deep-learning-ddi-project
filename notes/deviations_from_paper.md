...
**Independent confirmation this failure mode is real and not reproduction-specific:** the paper's own §4.1 ablation states — "We also trained the GNN model with zero-filled features (size 200) to evaluate the impact of informative and meaningful embeddings. The results show that zero features performed the worst, **with an AUC of 0.07**" — i.e. the authors' own model, in their own hands, produced an AUC far below random under a degraded configuration. This is strong independent evidence that this exact architecture (`GraphConv` + raw dot-product decoder + BCE) is capable of collapsing to badly sub-random AUC, not something specific to a translation error in this reproduction. **Exact numbers, from the paper's Table 5 "Zeros" row (user-supplied excerpt, 2026-09-08): AUC 0.0740 ± 0.0004, AUPRC 0.3355 ± 0.0000, AP@50 0.0000 ± 0.0000** — supersedes the rounded "0.07" quote above with the precise, citable figures.

**New (2026-09-08): a direct, same-encoder comparison — not just ChemBERTa — confirms the gap is not encoder-specific.** The paper's full Table 5 GNN block (user-supplied excerpt) reports every encoder it evaluated, not only the headline DeepChem ChemBERTa row:

| Encoder | Paper AUC | Paper AUPRC | Paper AP@50 |
|---|---:|---:|---:|
| DeepChem ChemBERTa | 0.9228 ± 0.0039 | 0.8944 ± 0.0025 | 0.9599 ± 0.0044 |
| ChemBERTa SimCSE | 0.9221 ± 0.0011 | 0.8929 ± 0.0007 | 0.7693 ± 0.0200 |
| GPT (small) | 0.9181 ± 0.0010 | 0.8900 ± 0.0003 | 0.8640 ± 0.0065 |
| Mol2vec | 0.8382 ± 0.0682 | 0.8152 ± 0.0604 | 0.8144 ± 0.0472 |
| **BERT** | **0.7951 ± 0.0463** | 0.8238 ± 0.0259 | 0.8879 ± 0.0084 |
| Doc2vec | 0.7273 ± 0.0174 | 0.7519 ± 0.0128 | 0.5146 ± 0.0555 |
| SBERT | 0.6745 ± 0.0040 | 0.7141 ± 0.0029 | 0.8170 ± 0.0094 |
| Zeros | 0.0740 ± 0.0004 | 0.3355 ± 0.0000 | 0.0000 ± 0.0000 |

This reproduction's side-effect nodes use `bert-base-uncased` (§ above, "What was implemented" bullet 1) — the same encoder as the paper's **BERT** row. That gives a literal same-encoder comparison, not a cross-encoder one: **paper BERT+GNN = 0.7951 ± 0.0463, this reproduction = 0.4113** — a gap of **8.3 std devs** (computed: (0.7951−0.4113)/0.0463) using the paper's own reported BERT variance, on the *exact* encoder pairing this reproduction actually used, independent of whether DeepChem ChemBERTa is special. This is stronger evidence than the Zeros-ablation comparison above that the unresolved gap is systemic to this reproduction's GNN training/evaluation, not an artifact of which encoder was chosen.
...
**Update 2026-09-09 (reopened at explicit user request; full detail in `notes/gnn_gap_diagnostics.md` "2026-09-09" section) — root cause of the training non-convergence identified: the authors' `GNN.py` declares `self.bn3`/`self.bn4` (`nn.BatchNorm1d`) but `forward()` never calls them.** The official reproduction faithfully omits them (dead code in the *released* source). Their absence leaves `conv3 → lin1` with no normalisation on an unnormalised `GraphConv` sum over side-effect degrees of 502–28,568 → unbounded logits → mean BCE ≈ 120 at epoch 1, training never converges (train AUC 0.589). Restoring `bn3` (`conv3 → bn3 → lin1`), in the legacy env, across seeds {42,0,1,2}: **training converges** (train loss 0.71 → 0.54, below the 0.693 baseline, all seeds) and test AUC rises to **0.66–0.73 under the paper's early-stopping criterion** (vs official 0.41). It still does **not** reproduce the paper's 0.9228: that value is reachable only via a very-early-epoch checkpoint whose AUC an *untrained* network matches on 2 of 4 seeds (0.94–0.95). **Verdict: the released code does not reproduce the reported GNN performance; the gap is largely a released-code bug (missing `bn3`); and the reported metric on this negative-sampling protocol substantially reflects graph topology (positives concentrate on high-degree side-effect nodes) rather than learned structure.** `bn3` is NOT adopted into `models/gnn.py` — faithfully implementing the released code, and documenting what it omits, is the finding. Re-frozen 2026-09-09.
