...
**Status: investigated in depth, root cause narrowed but not fully resolved.** Per explicit user instruction, no hyperparameter/architecture search was performed — every experiment above (BN-mode check, sign check, magnitude/angle decomposition, cosine-decoder retrain) is a targeted diagnostic reusing the existing trained checkpoint or an isolated single retrain to test one specific hypothesis, consistent with this repository's permanent "no hyperparameter search" constraint and the same controlled-ablation pattern as §1.8/§1.9. The most precise honest statement: the model learns a real, exploitable magnitude/popularity signal but fails to learn the angular/relational structure a dot-product decoder needs, for reasons not fully isolated (candidates, not tested: longer training budget, gradient clipping, the unresolved PyTorch/PyG version gap, or an initialization/random-seed sensitivity — the paper's own zero-features ablation shows this exact architecture is capable of this failure mode even for the original authors).
...
| GNN implemented but doesn't reproduce paper performance | Confirmed (§1.4, Milestone 10, 2026-08-11) | Architecture verified faithful to paper Eq. 1/2 and authors' source (one real bug found+fixed: negative-sampling local/global index mismatch); trains stably but test AUC 0.41 (paper 0.92, worse than random), AUPRC 0.53 (paper 0.89). 7-point root-cause investigation: model learns a real magnitude/popularity signal (AUC 0.82 using embedding norm alone) but no angular/relational signal (cosine similarity alone: AUC 0.51, uninformative) — dot product multiplies them, and a near-constant negative cosine inverts the otherwise-useful magnitude ranking. Cosine-decoder retrain tested as a fix and failed (AUC unchanged, AP@50 got worse), pointing the root cause upstream into encoder training dynamics. Paper's own zero-features ablation reports AUC=0.07, confirming this exact architecture can produce sub-random AUC even for the original authors. Unresolved: PyTorch 2.4.1/PyG 2.6.1 (paper) vs 2.12.1/2.8.0.post1 (repro) version gap |
